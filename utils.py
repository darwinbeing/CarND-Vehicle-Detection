...
def load_data(data_dir):
    cars = glob.glob(os.path.join(data_dir, 'vehicles/**/*.png'), recursive=True)
    notcars = glob.glob(os.path.join(data_dir, 'non-vehicles/**/*.png'), recursive=True)
    ...
    print("Total number of samples:", len(cars) + len(notcars))
...
def draw_labeled_bboxes(img, labels):
    ...
    for car_number in range(1, labels[1]+1):
        ...
        cv2.putText(img,'Vehicle',(bbox[0][0]+10, bbox[0][1]-5), font, 0.6,(255,255,255),2)

    # Return the image
    return img

def image_overlay(src, dst, alpha=0.5, ratio=0.25, origin=(0, 0)):
    image = cv2.resize(src=src, dsize=(0, 0), fx=ratio, fy=ratio, interpolation=cv2.INTER_AREA)
    ysize = image.shape[0]
    xsize = image.shape[1]
    backGround = dst[origin[1]:origin[1] + ysize, origin[0]:origin[0] + xsize]
    blend = np.round(backGround * (1 - alpha), 0) + np.round(image * alpha, 0)
    blend = np.minimum(blend, 255)
    dst[origin[1]:origin[1] + ysize, origin[0]:origin[0] + xsize] = blend
